...
--quais notas foram emitidas no mês de set de 2021
select*
from notas
where data_emissao BETWEEN '2000-01-01' and '2021-09-30'
order by data_emissao desc

--mostrar o cliente de cada nota mais recentenotas_itens
select notas.id, data_emissao, id_cliente, clientes.nome
from notas
	inner join clientes
...
--faça um sql quais clientes que mais compraram produtosnotas_itens
select id_cliente, count(id)
from notas
group by id_cliente
...
SELECT notas_itens.id_produto, produtos.descricao, SUM(notas_itens.quantidade) as quantidade_total
FROM notas_itens
	inner join produtos
    on notas_itens.id_produto=produtos.id
GROUP BY notas_itens.id_produto, produtos.descricao
