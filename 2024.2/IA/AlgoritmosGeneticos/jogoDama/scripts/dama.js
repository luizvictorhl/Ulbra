...
function updateScoreboard() {
    const pieceCount = getPieceCount(currentBoard);
    const redLabel = `Red: ${pieceCount.red}`;
    const blackLabel = `Black: ${pieceCount.black}`;
    ...
    if (winner !== 0) {
        const winnerLabel = winner === player ? "Jogador Vermelho venceu!" : "Computador venceu!";
        alert(winnerLabel); // Exibe um alerta
        d3.select("#winner").html(winnerLabel);
        currentBoard.gameOver = true; // Finaliza o jogo
    }
}
...
function highlightMoves(piece) {
    const moves = get_available_piece_moves(currentBoard, piece, currentBoard.turn);
    d3.selectAll("rect")
        .style("stroke", "none"); // Limpa destaques anteriores

    moves.forEach(move => {
        const cell = currentBoard.cells[getCellIndex(move.to.row, move.to.col)];
        d3.selectAll("rect")
            .filter(d => d.row === cell.row && d.col === cell.col)
            .style("stroke", "gold")
...
function moveCircle(cell, moveNum) {
    const cellCoordinates = mapCellToCoordinates(board_origin, cell_width, cell);
    currentBoard.delay = (moveNum * 500) + 500;
    d3.selectAll("circle").each(function(d) {
        if (d.col === cell.col && d.row === cell.row) {
            d3.select(this)
                .transition()
                .duration(300) // Adiciona suavidade
                .attr("cx", d.x = cellCoordinates.x + cell_width / 2)
                .attr("cy", d.y = cellCoordinates.y + cell_width / 2);
        }
    });
...
function drawBoard(origin, cellWidth, boardCanvas) {
    ...
    .data(cells)
    .enter().append("rect")
    .attr("x", d => mapCellToCoordinates(origin, cellWidth, d).x)
    .attr("y", d => mapCellToCoordinates(origin, cellWidth, d).y)
    .attr("height", cellWidth)
    .attr("width", cellWidth)
    .style("fill", d => (d.row + d.col) % 2 === 0 ? "#f0d9b5" : "#b58863")
    .style("stroke", "black")
